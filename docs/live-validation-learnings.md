...
| 2026-09-25 | The totals-collector webhooks work end to end: Commerce's real payload carries the buyer's customer group, the ERP resolves the company from it, and the price update lands. They need the company in a group of its own (see above) and room for a cold start: Commerce now waits 10 s and every run is recorded. | Cart priced at the 40 contract price at 13:52; runs of 2.6–2.8 s recorded in Runtime; the earlier miss under the 5 s limit left no record | `test/box/journeys.test.js` (pricing journeys); `webhooks.test.js` | `docs/eventing.md`, `docs/demo-setup.md` |
