...
| Settings | per website or store view, kept by App Management's business configuration: send orders, hold orders while offline, mark Processing on confirm, contract prices, discount ceiling; and the **Structure** group below. On the Admin screen each one sits on the card of the entity it joins (the **Mapping** tab, below) | `erp/settings`, `src/lib/settings.js` |
